...
class LemurRevise extends StatefulWidget {
  const LemurRevise({super.key});

  @override
  State<LemurRevise> createState() => _LemurReviseState();
}

class _LemurReviseState extends State<LemurRevise> {
  ...
  // image.network  
  String baseUrl = "https://cais.cbinstrument.com/";
  String approvalPhotoUrl = "";
  List<String> workingPhotoUrls = [];
  ...
  Future<void> _fetchingRevise() async {
    _isLoading = true;
    if (_token == null) {
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => RackupAbsence()));
      return;
    }
    ...
    if (response.statusCode == 200) {
      ...
      print(bodi);
      setState(() {
         workingListCtrl.text = bodi["daftar_pekerjaan"];
         lemburW1 = bodi["lembur_weekday_1"];
         w1Ctrl.text = bodi["lembur_weekday_1"].toString();
         lemburW2 = bodi["lembur_weekday_2"];
         w2Ctrl.text = bodi["lembur_weekday_2"].toString();
         lemburWE1 = bodi["lembur_weekend_1"];
         we1Ctrl.text = bodi["lembur_weekend_1"].toString();
         lemburWE2 = bodi["lembur_weekend_2"];
         we2Ctrl.text = bodi["lembur_weekend_2"].toString();
         lemburWE3 = bodi["lembur_weekend_3"];
         we3Ctrl.text = bodi["lembur_weekend_3"].toString();
         _cancelation = bodi["keterangan"];
         approvalPhotoUrl = "$baseUrl${bodi["bukti_persetujuan_atasan"]}";
         workingPhotoUrls =
          List<String>.from(
            bodi["bukti_pekerjaan"].map(
              (e) => "$baseUrl$e",
            ),
          );
        _isLoading = false;
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Text(
            "Report Has been Sent, and will be checked by HR :)",
          ...
          children: [
            ...
            SizedBox(
              ...
              Container(
                ...
                child: 
                Card(
                  color: Color.fromRGBO(253, 75, 75, 0.2),
                  shape: RoundedRectangleBorder(
                  borderRadius: BorderRadius.circular(12),
                  side: BorderSide(
                    color: Color.fromRGBO(255, 89, 89, 0.4),
                    width: 1,
                  ),
                ),
                child: 
                  Center(child: 
                    Row(
                      ...
                      children: [
                        Icon(MaterialCommunityIcons.alert, color: Color.fromRGBO(255, 82, 82, 0.8)),
                        Padding(
                          padding: const EdgeInsets.only(left: 5.0),
                          child: Text("$_cancelation", style: TextStyle(color: Color.fromRGBO(255, 82, 82, 0.8)),),
                        ),
                      ],
                    )
                  ),
                  ...
                  child: Column(
                    children: [
                      ...
                      ElevatedButton(
                        ...
                        child: Text(t.translate('setToPrefs')),
                      ),
                      // )
                    ],
                  ),
                        ...
                        : workingPhotoUrls.isNotEmpty
                            ? SizedBox(
                                ...
                                child: ListView.builder(
                                  ...
                                  itemCount: workingPhotoUrls.length,
